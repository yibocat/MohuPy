...
def normalize(d1, d2, t=1.):
    ...
    """
    # Built-in function for calculating standardized elements obtained by risk factors
    from ..core.mohu import MohuQROHFN
    assert isinstance(d1, MohuQROHFN) and isinstance(d2, MohuQROHFN), \
        "normalize function just adapt q-rung orthopair hesitant fuzzy numbers."

    adj = lambda d, tm: tm * d.max() + (1. - tm) * d.min()
    ...
    if md_len > 0:
        ...
        m = d_2.md
        while i < md_len:
            d_2.md = np.append(d_2.md, adj(m, t))
            i += 1
    else:
        ...
        m = d_1.md
        while i < (-md_len):
            d_1.md = np.append(d_1.md, adj(m, t))
            i += 1
    ...
    if nmd_len > 0:
        ...
        u = d_2.nmd
        while i < nmd_len:
            d_2.nmd = np.append(d_2.nmd, adj(u, t))
            i += 1
    else:
        i = 0.
        u = d_1.nmd
        while i < (-nmd_len):
            d_1.nmd = np.append(d_1.nmd, adj(u, t))
            i += 1
